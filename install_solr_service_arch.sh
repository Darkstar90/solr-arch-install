...
if [ $? -ne 0 ]; then
  echo "Creating new user: $SOLR_USER"
    useradd -d "$SOLR_VAR_DIR" -r -M -s /bin/bash "$SOLR_USER"
fi
...
echo -e "\nInstalling /etc/systemd/system/$SOLR_SERVICE.service script ...\n"

cat <<EOT >> /etc/systemd/system/$SOLR_SERVICE.service
[Unit]
...
Group=solr
PrivateTmp=true
Environment=/etc/default/solr.in.sh
ExecStart=/opt/solr/bin/solr start -a '-Xloggc:/opt/solr/server/logs/solr-garbage-collector.log'
...
chmod 0744 "/etc/systemd/system/$SOLR_SERVICE.service"
...
sed -i -e "$sed_expr1" -e "$sed_expr2" -e "$sed_expr3" -e "$sed_expr4" "/etc/systemd/system/$SOLR_SERVICE.service"
...
# set file permissions on install directory
chown -R solr:solr /opt/solr-6.6.0

# start service
if [[ $SOLR_START == "true" ]] ; then
  systemctl start "$SOLR_SERVICE" 
  sleep 5
  systemctl -l status "$SOLR_SERVICE" 
else
